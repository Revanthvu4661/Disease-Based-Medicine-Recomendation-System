...
async function loadStats() {
  try {
    ...
    if (stats.byDisease && stats.byDisease.length) {
      ...
      document.getElementById('diseaseBars').innerHTML = stats.byDisease.map(d => `
      ...
      document.getElementById('diseaseBars').innerHTML = `<div class="no-data"><i class="fas fa-chart-bar"></i><p>No data yet</p></div>`;
    }
  } catch (err) {
    console.error('Stats error:', err);
    // If the user is not actually authorized as a doctor, make the failure obvious.
    if (String(err.message || '').toLowerCase().includes('doctors only')) {
      showToast('Session role mismatch. Please logout and login as Doctor.', 'error');
    }
  }
}
...
async function loadPendingRecords() {
  try {
    const res = await api.get('/records?reviewed=false&limit=1000');
    renderCardGrid('pendingArea', res.records || res, false);
  } catch (err) { console.error(err); }
}

async function loadSevereRecords() {
  try {
    const res = await api.get('/records?severity=severe&limit=1000');
    renderCardGrid('severeArea', res.records || res, false);
  } catch (err) { console.error(err); }
}
...
async function loadReviewedRecords() {
  ...
  try {
    const res = await api.get('/records?reviewed=true&limit=1000');
    renderCardGrid('reviewedArea', res.records || res, false);
  } catch (err) {
...
async function loadDeletedRecords() {
  ...
  try {
    const res = await api.get('/records/deleted');
...
async function openModal(id) {
  ...
  try {
    ...
    document.getElementById('modalBody').innerHTML = `
      ...
      ${r.cured ? `
      <div style="margin-top:16px;padding:12px 16px;background:rgba(34,197,94,0.08);border:1px solid rgba(34,197,94,0.3);border-radius:10px;">
        <div style="color:#22c55e;font-weight:600;font-size:0.9rem;"><i class="fas fa-check-double"></i> Patient Reported Cured</div>
        <div style="color:var(--text-muted);font-size:0.85rem;margin-top:6px;">Marked as cured on ${new Date(r.curedAt).toLocaleDateString('en-IN')}</div>
      </div>` : ''}
    `;
  } catch (err) {
...
async function deleteRecord() {
  if (!currentRecordId) return;
  try {
    await api.patch(`/records/${currentRecordId}/delete`, {});
    closeModal('patientModal');
    loadStats();
    loadAllRecords();
    loadPendingRecords();
...
function renderDiseaseChart(byDisease) {
  const ctx = document.getElementById('diseaseChartCanvas');
