...
async function showHistoryDetail(recordId) {
  try {
    ...
    const curedBtn = document.getElementById('curedBtn');
    ...
    body.innerHTML = `
      <div style="display: grid; gap: 16px;">
        ...
        ${record.cured ? `
        <div style="background:rgba(16,185,129,0.1);padding:12px 16px;border-radius:8px;border:1px solid rgba(16,185,129,0.3);">
          <div style="color:var(--success);font-weight:600;"><i class="fas fa-check-double"></i> Status: Cured</div>
          <div style="color:var(--text-muted);font-size:0.85rem;margin-top:4px;">Marked as cured on ${new Date(record.curedAt).toLocaleDateString()}</div>
        </div>` : ''}
      </div>
    `;
    curedBtn.style.display = record.cured ? 'none' : 'block';
...
async function markAsCured() {
  if (!selectedRecordId) return;
  if (!confirm('Mark this disease as cured? Your doctor will be notified.')) return;

  try {
    await api.patch(`/records/${selectedRecordId}/cured`, {});
    closeModal('historyDetailModal');
    showToast('✅ Disease marked as cured', 'success');
    loadHistory();
  } catch (err) {
    showToast('Error marking as cured: ' + err.message, 'error');
  }
}
...
function exportHistoryCSV() {
  const records = filteredHistory.length > 0 ? filteredHistory : cachedRecords;
